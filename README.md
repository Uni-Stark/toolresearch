...
3. 파일의 버전관리가 쉽다.
	...
	> <img src="./markdown/img/git_repo_mgmt.png"/>
...
> 업무용 툴은 하나만 쓴다고 다 해결되는 것이 아니다. 물론 하나로 해결이 다 가능하면 정말 좋겠지만, 그게 힘들기 때문에 각 툴들도 다른 업무용 툴과 연결할 수 있는 기능을 제공하는 것이다. 내 개인적인 생각으로는 우리 회사에서 일단 도입되었으면 하는 툴들은 Git, Trello, Slack 이 3가지다.
> Git은 `개발을 하는데 효율성을 증진시키는데 가장 필요한 툴`이기 때문에 꼽았다. 그리고 Git과의 연동성을 고려했을 때 Trello와 Slack을 함께 쓰면 시너지도 좋기에 Git은 꼭 도입되었으면 하는 툴이다.
> Trello는 개발을 해야할 것, 하는 것, 한 것들에 대한 `가시적 효과`가 뛰어나고, `사용하기가 편하다`는 점때문에 도입을 원하는 것이다.
