...
> 콜라비는 우리회사랑 잘 어울린다. 왜냐하면 툴을 왔다갔다하는 시간, 툴 사용법을 익히는 시간을 줄여주기 때문이다. 여러가지 사용할 필요없이 하나의 툴에서 해결할 수 있다면 이보다 좋은 상황은 없는거 아닌가 하는 생각이 든다. 다만 한가지 걸리는게 있다면, 많은 기능들이 아직 개발 진행중이라는 점이다. 물론 완벽한 툴은 없지만 2%의 부족함을 보이는 툴인 것은 명백하다.